...
def test_speedlimits_points_builder_correct_inputs():
    ...
    points = spdlts.speedlimits_points_builder(list_of_points)
...
def test_format_speedlimits_query_valid_points_default_inputs():
    ...
    check = "Routes/SnapToRoad?includeSpeedLimit=true&travelMode=driving&" + \
            "points={}&speedUnit={}".format(points,
                                            'KPH')
    assert(query == check)
...
def test_parse_speedlimits_data_successful_response():
    ...
    assert({'Latitude', 'Longitude', 'Street',
            'Speed Limit', 'Speed Unit'}.issubset(data.columns))
    # code below checks if any non-null value is Main Street
    # should be true!
    assert(data.isin([70]).any().any())