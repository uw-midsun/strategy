...
def calculateSOCValues(v_profile, e_profile, distance, initial_soc):
    ...
    last_point_velocities = [v_profile[len(v_profile) - 1], v_profile[len(v_profile) - 1]]
    last_elevation = [e_profile[len(e_profile) - 1], e_profile[len(e_profile) - 1]]
    last_energy_used = car_model.energy_used(last_point_velocities, last_elevation, distance[len(distance) - 1])
...
initial_soc = 1
generateSOCGraph(velocities, elevations, distances, initial_soc)